...
function SkillCard({ skill, darkMode }) {
  ...
  return (
    <div className="relative px-2 py-4 md:px-4 md:py-6 flex flex-col items-center gap-2.5 md:gap-3">
      ...
      <span className={`font-medium text-xs md:text-sm text-center transition-colors duration-300 ${darkMode ? 'text-gray-200' : 'text-black'
        }`}>
        {skill.name}
      </span>
    </div>
...
export default function Skills({ darkMode, lang }) {
  ...
  return (
    <section
      id="skills"
      className={`py-10 px-5 md:px-10 transition-colors ${darkMode ? 'bg-gray-900 text-white' : 'bg-white text-black'
        }`}
    >
      <div className="max-w-7xl mx-auto">
        ...
        <div className="text-center mb-4">
          <h2
            className={`relative pb-4 inline-block font-display font-bold text-2xl md:text-3xl ${darkMode ? 'text-white' : 'text-black'
              }`}
          >
        ...
        <div className="space-y-6 mt-6">
          {Object.entries(groupedSkills).map(([categoryName, categorySkills]) => (
            <div
              key={categoryName}
              className={`rounded-2xl p-4 md:p-8 transition-colors ${darkMode ? 'bg-gray-800/50' : 'bg-white border border-gray-100'
                }`}
            >
              <h3 className={`text-lg md:text-xl font-bold mb-4 md:mb-6 pb-2 inline-block border-b-2 ${darkMode ? 'text-white border-[#ff5e3a]' : 'text-navy-600 border-black'
                }`}>
                {categoryName}
              </h3>
