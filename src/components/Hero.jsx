...
export default function Hero() {
  const [visitorCount, setVisitorCount] = useState('237')

  useEffect(() => {
    // These keys act as your database table on the free cloud counter service
    const namespace = 'my-github-portfolio-tracker-v1'
    const key = 'total-visits-v2' // Changed key to ensure a fresh counter instance

    // Prevent counting the same user multiple times if they refresh during the same session
    const hasVisitedSession = sessionStorage.getItem('hasVisitedSession')

    const fetchVisitorCount = async () => {
      try {
        if (!hasVisitedSession) {
          // Hits/increments the counter by +1 for a new unique session
          const response = await fetch(`https://api.countapi.xyz/hit/${namespace}/${key}`)
          const data = await response.json()
          
          // If the API starts at 0, we can add 237 to make 237 the baseline
          const baseCount = data.value + 236 
          setVisitorCount(baseCount)
          sessionStorage.setItem('hasVisitedSession', 'true')
        } else {
          // Just fetches the current total without incrementing if they already loaded it
        ...
        // Fallback display if offline or network blocks the request
        setVisitorCount(237)
      }
    }
  ...
  return (
    ...
    >
      {/* Faster, more dynamic zodiac/constellation starfield background accent */}
      <svg
        className="absolute inset-0 w-full h-full opacity-[0.12] pointer-events-none"
        viewBox="0 0 800 600"
        preserveAspectRatio="xMidYMid slice"
      >
        <g className="animate-[spin_40s_linear_infinite]" style={{ transformOrigin: '400px 300px' }}>
          {Array.from({ length: 22 }).map((_, i) => (
            <circle
              key={`star-${i}`}
              cx={(i * 137) % 800}
              cy={(i * 91) % 600}
              r={i % 3 === 0 ? "3.5" : i % 2 === 0 ? "2.5" : "1.5"}
              fill="#1f2f5c"
            />
          ))}
        ...
        <g className="animate-[spin_25s_linear_infinite_reverse]" style={{ transformOrigin: '400px 300px' }}>
      ...
      <div className="relative max-w-7xl mx-auto">
        {/* Real Live Visitor Count Badge displayed above profile */}
        <div className="flex justify-center mb-8">
          <div className="inline-flex items-center gap-2 px-4 py-1.5  text-navy-700 text-xs md:text-sm font-medium ">
        ...
        <div className="grid md:grid-cols-2 gap-12 items-center">
          ...
          <div>
            ...
            <p className="font-display font-bold text-3xl sm:text-4xl md:text-5xl leading-tight mt-1">
              I'am <span className="text-amber-500">{profile.name}</span>
            </p>
